...
LOG_LEVEL=DEBUG uvicorn vibeword.main:app --reload
```
...
**VibeWord** is a real-time collaborative crossword solver. Players share a room, see each other's cursors, and solve together.

### Backend (`src/vibeword/`)
...
- **`config.py`** — Config is loaded from `config.yaml` (or the path in `VIBEWORD_CONFIG`), then overridden by env vars of the form `VIBEWORD_<SECTION>_<KEY>` (e.g. `VIBEWORD_UI_HOLD_DELAY_MS`). Sections: `server`, `room`, `ui`.
